...
//------------------------Parameter----------------------
`define TV_IN_threshold "../tv/cdatafile/c.qr_binarize.autotvin_threshold.dat"
`define TV_IN_invert "../tv/cdatafile/c.qr_binarize.autotvin_invert.dat"
`define TV_IN_size "../tv/cdatafile/c.qr_binarize.autotvin_size.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter threshold_DEPTH = 1;
reg [31 : 0] threshold_OPERATE_DEPTH = 0;
parameter threshold_c_bitwidth = 8;
parameter invert_DEPTH = 1;
reg [31 : 0] invert_OPERATE_DEPTH = 0;
parameter invert_c_bitwidth = 1;
parameter size_DEPTH = 1;
reg [31 : 0] size_OPERATE_DEPTH = 0;
parameter size_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter qr_binarize_continue_addr = 0;
parameter qr_binarize_auto_start_addr = 0;
parameter threshold_data_in_addr = 16;
parameter invert_data_in_addr = 24;
parameter size_data_in_addr = 32;
...
reg [DATA_WIDTH - 1 : 0] mem_invert [invert_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_invert [ (invert_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * invert_DEPTH -1 : 0] = '{default : 'hz};
reg invert_write_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_size [size_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_size [ (size_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * size_DEPTH -1 : 0] = '{default : 'hz};
...
//write invert reg
reg [31 : 0] write_invert_count = 0;
reg write_invert_run_flag = 0;
reg write_one_invert_data_done = 0;
//write size reg
...
assign TRAN_CTRL_write_data_finish = 1 & threshold_write_data_finish & invert_write_data_finish & size_write_data_finish;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_threshold_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_threshold[write_threshold_count * four_byte_num  + i]!==threshold_data_tmp_reg) begin
                    write (threshold_data_in_addr + write_threshold_count * four_byte_num * 4 + i * 4, threshold_data_tmp_reg, write_threshold_resp);
                    image_mem_threshold[write_threshold_count * four_byte_num + i]=threshold_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        invert_write_data_finish <= 0;
        write_invert_run_flag <= 0; 
        write_invert_count = 0;
        count_operate_depth_by_bitwidth_and_depth (invert_c_bitwidth, invert_DEPTH, invert_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_CTRL_start_in === 1) begin
            invert_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_invert_run_flag <= 1; 
            write_invert_count = 0;
        end
        if (write_one_invert_data_done === 1) begin
            write_invert_count = write_invert_count + 1;
            if (write_invert_count == invert_OPERATE_DEPTH) begin
                write_invert_run_flag <= 0; 
                invert_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_invert
    integer write_invert_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] invert_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = invert_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_invert_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write invert data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (invert_c_bitwidth < 32) begin
                        invert_data_tmp_reg = mem_invert[write_invert_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < invert_c_bitwidth) begin
                                invert_data_tmp_reg[j] = mem_invert[write_invert_count][i*32 + j];
                            end
                            else begin
                                invert_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_invert[write_invert_count * four_byte_num  + i]!==invert_data_tmp_reg) begin
                    write (invert_data_in_addr + write_invert_count * four_byte_num * 4 + i * 4, invert_data_tmp_reg, write_invert_resp);
                    image_mem_invert[write_invert_count * four_byte_num + i]=invert_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_invert_data_done <= 1;
                @(posedge clk);
                write_one_invert_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_size_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_size[write_size_count * four_byte_num  + i]!==size_data_tmp_reg) begin
                    write (size_data_in_addr + write_size_count * four_byte_num * 4 + i * 4, size_data_tmp_reg, write_size_resp);
                    image_mem_size[write_size_count * four_byte_num + i]=size_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
// Read data from file 
initial begin : read_invert_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [invert_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (invert_c_bitwidth , factor);
  fp = $fopen(`TV_IN_invert ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_invert); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < invert_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_invert [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < size_DEPTH; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_size [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
